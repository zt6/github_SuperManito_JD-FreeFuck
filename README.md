...
ㅤ同步上层源码更新了 `格式化导出互助码脚本`，现在更好用了；优化了控制面板的排版和底层代码，请所有朋友执行 `git_pull` 一键更新脚本，然后根据对应版本《使用与更新》教程之<控制面板教程>的第6条命令升级，最后根据第3条命令重启控制面板完成更新。本项目前致力于开发控制面板，遇到问题请立即反馈，感谢您的理解与支持。

- __2021/3/09 06:00 `更新与修复`__\
ㅤ1. 更新了控制面板，手动执行模块新增多个功能 \
ㅤ2. 修复了 `Linux` 版无法使用控制面板手动执行模块的问题 \
ㅤ3. 变更了一键执行所有活动脚本的命名，注意今后使用新命令 \
ㅤ请所有朋友执行 `git_pull` 一键更新脚本；`Linux` 版用户还请根据《使用与更新》教程之<更新教程>的第3命令手动修复上述问题，`Docker ` 版用户不需要执行此命令；最后请根据对应版本《使用与更新》教程之<控制面板教程>的第3条命令重启控制面板完成更新。此次更新功能较多，如果遇到问题再尝试重启控制面板无效后请及时向我反馈，感谢您的支持。

- __2021/3/08 22:30 `更新`__
ㅤ更新了配置文件，版本号到`3.32.1`，新增 "环球挑战赛" 互助码类环境变量，新增 "定义宠汪汪赛跑是否开启本地账号内部互助" 、 "定义京小兑是否将抽奖卷自动兑换为兑币" 环境功能控制变量，修正了部分注释内容。您可根据对应版本《使用与更新》教程之<更新教程>的第2命令手动更新替换配置文件，您也可以通过控制面板对比工具自行修改。
