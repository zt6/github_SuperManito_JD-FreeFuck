...
> _注意：1. 此脚本为本人原创的一键更换国内更新源脚本，脚本自带备份功能，无需手动备份原有官方源，_\
> _ㅤㅤㅤ2. 此脚本文件链接调用本人的另一个原创项目，之前附加在旧版本部署脚本中现独立出来，_\
> _ㅤㅤㅤ3. 如果您使用的平台位于国外则不需要执行此脚本，例如 `Google Cloud Platform` 用户，_\
...
6. 如果您的网络环境暴露在了公网（例如 VPS 用户），请根据《使用与更新》教程更改 `控制面板` 的用户名和密码。
7. 若您使用 `VPS` 平台，关机前请先执行此命令手动停止控制面板，否则在仪表盘强制关机重启后可能会出现无法保存配置文件的问题。
...
> _参考命令：sed -i '30c Cookie1="pt_pin=xxxxx;pt_key=xxxxxxx;"' config/config.sh_
