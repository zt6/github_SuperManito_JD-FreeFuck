...
> _注意：`VPS` 平台用户关机前请先执行此命令手动停止控制面板，否则在仪表盘强制关闭重启后可能会出现无法保存配置文件的问题。_
#### 3. 重启控制面板：
...
ㅤ
## 五、使用需知
#### 1.  `run-all.sh` 为一键执行所有活动脚本， `git_pull.sh` 为一键更新脚本
...
#### 5. 如果需要更新活动脚本，请执行 `docker exec -it jd bash git_pull.sh` 命令一键更新即可，它会同步更新 `run-all.sh` 脚本
